...
# APPENDIX A — Protocol Amendment and Results Summary

**Added 2026-07-19. This appendix is post-hoc.** Sections 0–23 above are the
pre-registration and are deliberately **left unmodified**, including hypotheses whose
outcomes are now known. Everything below documents what was actually run, how it
deviated from the plan, and what remains outstanding. Full results:
[`STUDY_RESULTS.md`](STUDY_RESULTS.md); audit detail: [`MENTOR_MEMO.md`](MENTOR_MEMO.md).

## A.1. What was executed

| Protocol element | Planned | Executed |
|---|---|---|
| Design | A (VLM sees image, report held out) | ✅ as planned |
| Dataset | IU X-Ray / OpenI (compliance §0.2) | ✅ 3,826 studies |
| Test set | ~250–350 paired cases (§ power) | ✅ **754**, balanced over 6 strata |
| Conditions | 5 prompts | ✅ all 5, full 754 each (3,770 generations) |
| Model | capable VLM | gemini-2.5-flash, T=0, full-res, hidden CoT disabled |
| Reference | CheXbert primary | ✅ CheXbert (`lblcx_`) |
| Accuracy / F1 | §7.1 | ✅ |
| Groundedness | §7.2 | ✅ claim-level, 4-state |
| Calibration | §7.3 | ✅ ECE / mean-conf / overconfidence gap |
| Uncertain-label variants | §7.1 (U-ones/zeros/ignore) | ✅ all three; ranking stable |
| **LLM judge** | §8 | ✅ **re-run 2026-07-19 after fix — 3,770/3,770 scored, 0 errors** |
| Human sanity check | §9 | ❌ not performed |

## A.2. Headline results

Micro-F1 vs CheXbert, hierarchy-aware, uncertain-as-positive (n=754):

| Condition | 13-class | reliable-11 | 3-group | binary (dx only) | binary (full resp.) |
|---|---|---|---|---|---|
| direct | 0.302 | 0.323 | 0.412 | 0.559 | 0.683 |
| cot | 0.333 | 0.356 | 0.444 | 0.596 | 0.703 |
| ddx | 0.307 | 0.330 | 0.415 | 0.627 | 0.691 |
| evidence_first | **0.367** | **0.391** | **0.482** | 0.585 | 0.711 |
| uncertainty_first | 0.359 | 0.384 | 0.476 | 0.621 | **0.712** |

Paired McNemar (top-1 accuracy, vs evidence_first): significantly beats `direct`
(p=0.0000) and `cot` (p=0.0104); statistically tied with `ddx` (p=0.32) and
`uncertainty_first` (p=0.87). **Conclusion: a top tier of three, not a single winner.**

### Hypothesis outcomes

| | Pre-registered claim | Outcome |
|---|---|---|
| **H1** | Evidence-first improves reference-consistency | **Not supported as stated.** `direct` has the highest support rate (0.850 vs 0.821) — but makes only 2.04 claims/case vs evidence_first's 4.60. Reframed as a coverage/fidelity tradeoff: evidence_first surfaces 2.25× more evidence at near-equal fidelity and the lowest contradiction rate among reasoning prompts (0.075). |
| **H2** | Uncertainty-first reduces overconfidence | **Supported.** Lowest ECE (0.641), lowest mean confidence (0.870), smallest gap. Secondary finding: *all* conditions severely overconfident (gap +0.64 to +0.71). |
| **H3** | CoT/DDx more convincing but not more faithful | **Supported, tested as pre-registered.** Paired-Wilcoxon dissociation on judge scores: `cot` coherence +0.248 (p<0.0001) with faithfulness **−0.229** (p<0.0001); `evidence_first` +0.131 (p=0.0020) with faithfulness −0.121 (p=0.0003). Faithfulness actively *declined* rather than merely staying flat — stronger than hypothesised. `ddx`/`uncertainty_first` do not dissociate (lower on both axes). Corroborated by the independent claim-level support rate. |
| **H4** | Prompt effects larger on ambiguous cases | **Supported.** Prompt-effect spread largest on hard_multi (0.173) and rare (0.144), smallest on easy_abnormal (0.050). Tested by cross-stratum spread, not the pre-registered GLMM interaction term (A.3). |

## A.3. Deviations from protocol

1. **Hierarchy-aware scoring (added).** §7.1 specified flat 14-class mapping. Flat
   scoring double-penalises a correct parent/child match (model "Lung Opacity" vs
   report "Atelectasis") as both FP and FN — 26% of all false positives. Parent
   propagation on both sides is standard CheXpert practice and was adopted.
2. **Granularity ladder (added).** Results are reported at 13-class, reliable-11,
   3-group, and binary levels rather than 14-class alone, because two classes have
   inter-labeler κ < 0.4 and are not reliably measurable. Groups follow the standard
   CXR review framework; the full ladder is always reported together.
3. ~~**Uncertain-label policy (narrowed).**~~ **Resolved 2026-07-19.** All three
   §7.1 variants are now reported (`scripts/20_uncertain_variants.py`,
   `results/study_uncertain_variants_test.csv`). Headline numbers remain U-ones; the
   **prompt ranking is identical under U-ones, U-zeros and U-ignore** (max spread
   0.017), so conclusions are robust to the convention. A latent bug was fixed in the
   process: `u_policy='ignore'` had been computed identically to `'zeros'`, because a
   positive-set return cannot express "cell excluded" — true U-ignore now masks
   reference-uncertain cells out of the confusion matrix.
4. **Secondary binary operationalization (added).** Alongside the pre-registered
   "final diagnosis vs label" metric, a triage metric parses diagnosis + explanation.
   Reported as a clearly separate column; the two are not interchangeable.
5. ~~**H3 test substituted.**~~ **Resolved 2026-07-19:** the judge was fixed and
   re-run, so H3 was tested exactly as pre-registered (paired-Wilcoxon dissociation,
   `scripts/19_h3_dissociation.py`). The contradiction-rate analysis is retained as a
   corroborating secondary measure.
6. **H4 test substituted.** Pre-registered as a mixed-effects interaction term;
   executed as cross-stratum spread in 3-group F1. The GLMM is *outstanding*.
7. **Exploratory prompts (added, out of scope).** Four additional prompts
   (`structured`, `structured_balanced`, `structured_soft`, `structured_targeted`)
   and two decoding strategies (self-consistency ensembling, chain-of-verification)
   were explored while investigating score levels. **These are not part of the
   pre-registered five-condition comparison** and belong in a methods note, not the
   main result.

## A.4. Known gaps and failures

- ~~**LLM-judge (§8) effectively did not execute.**~~ **RESOLVED 2026-07-19.** The
  original run scored only 48/3,770 rows (1.3%): the judge did not disable Gemini 2.5
  hidden reasoning (empty responses), had no retry, and a bare `except Exception`
  silently converted every failure to NaN — so a failed run looked complete. After the
  fix (`src/judge.py`), the re-run scored **3,770/3,770 with 0 errors**. The failed
  artefacts are quarantined in `results/invalid/` with a README. The §7.4 rubric and
  the H3 dissociation test are now executed.
- **Stale statistics files.** `results/stats_*.csv` predate the evaluation audit and
  contain superseded numbers. Authoritative outputs are
  `results/study_{performance,calibration,difficulty}_test.csv`.
- **Human sanity check (§9) not performed.**
- **Single model.** gemini-2.5-pro was spot-checked only.

## A.5. Evaluation audit (2026-07-19)

Six defects were found in the evaluation code and corrected; fine-grained F1 for all
five conditions roughly doubled (0.19–0.27 → 0.30–0.37) as a result. Note that these
raised the *measurement*, not the model.

| # | Defect | Effect |
|---|---|---|
| 1 | Flat scoring ignored the CheXpert hierarchy (26% of FPs) | large gain |
| 2 | Scored against impression-only labels, not CheXbert | large gain |
| 3 | `evidence_first`'s `extracted_evidence` / `ddx`'s `differential` discarded | evidence_first +0.027 (3-group) |
| 4 | No negation scoping in the prediction mapper ("No evidence of pneumonia" → positive) | *lowered* scores; kept, it is correct |
| 5 | Lexicon gaps ("congestive heart failure" mapped to nothing) | mixed/small |
| 6 | Report labeler used sentence-level negation scoping (148 mentions) | mean κ 0.640→0.645 |

Verified clean: prediction↔label alignment (permutation test z=18 vs chance),
image↔study correspondence, manual end-to-end case traces.

**Rejected as invalid:** greedy class-subset selection (0.636 by keeping only Lung
Opacity — a one-class system) and dev-tuned group abstention (degenerated to one group,
scored *worse* held-out). Neither is reported.

## A.6. Interpretation ceiling

Two independent report labelers score only **F1 0.771** against each other (mean
κ 0.645; 68.3% exact case agreement). That bounds anything graded against a
report-derived reference. The model also reports **1.7× more findings per study** than
the terse IU reports mention. Distinguishing genuine over-calls from
visible-but-undictated findings requires expert image labels; a 200-study expert set is
specified and tooled in `pipeline/EXPERT_VALIDATION_SCOPE.md` but **not yet labeled**.

Accordingly, this study supports **comparative** claims about prompt style. It does not
support claims of clinical readiness or fine-grained labeling competence.
